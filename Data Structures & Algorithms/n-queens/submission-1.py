'''
Iterate through rows of the board, so that we never put two queens on the same row. 
Maintain a set for used columns, positive and negative diagonals, and skip cols on the current
row which are used in any of the three sets. Finally backtrack and try with the next col.
'''
class Solution:
