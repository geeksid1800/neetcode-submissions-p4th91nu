'''
similar to LRU, but maintain a linkedlist for each unique access 'count'.
E.g. keys accessed once will all be in the same LL, as will keys accessed twice in same LL.
Therefore it makes sense to define a LinkedList class, with each object having its own 
root and tail. 
Important point is that at each point, we have to maintain minCount, which is the access
count of the least accessed key(s). It will potentially be updated in two cases:
1) If we access a key, it moves from LL corresponding to 'cnt' to 'cnt+1'. In case the old
LL becomes empty, and minCount is currently equal to cnt, it means that key was the only one
with access count 'cnt' and now since it's increased, minCount also has to go up by 1
2) Whenever there is a call to put(), there is a chance we are adding a new key,(whose access
count will be 1, lesser than the existing minCount), so check if that's the case by
minCount = min(minCount,count[key])
'''
