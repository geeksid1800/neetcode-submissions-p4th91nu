...
recursively.
Optimization: Out of the 2^n paths/choices, there are many paths where one grp
is much larger than the other (e.g. all stones go to grp1), which are not useful
to our answer, as we have to minimize grp1-grp2. In fact, once grp1 crosses tot/2
the gap between grp1 and grp2 is just going to keep increasing, so it makes sense
to stop adding to grp1 once we reach ceil(tot/2).
'''
class Solution:
    def lastStoneWeightII(self, stones: List[int]) -> int:
        ...
        def recur(i,sum1):
            ...
            '''
            if i==len(stones) or sum1>=(tot+1)//2:
